...
export default class Star extends React.Component {
  ...
  getClassName() {
    ...
    if (value === 0 && focused) {
      className += ` ${prefixCls}-focused ${prefixCls}-zero`;
  ...
  render() {
    ...
    const { disabled, prefixCls, characterEmpty, characterFull, index, count, value } = this.props;
      ...
      <li
        ...
        tabIndex={0}
      >
        <div className={`${prefixCls}-first`}>{firstCharacter}</div>
        <div className={`${prefixCls}-second`}>{secondCharacter}</div>
      </li>
    );
  }
}
