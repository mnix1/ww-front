...
class IntroStepEditTeam extends React.PureComponent {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (stepId === INTRO_STEP_EDIT_TEAM && path === WISIES_TEAM_EDIT_ROUTE && prevProps.path === WISIES_ROUTE) {
            onStepIndexChange(STEP_ID_TO_NEXT_STEP_INDEX[stepId]);
        }
    }

    render() {
        return <IntroStep stepId={INTRO_STEP_EDIT_TEAM} renderContinue={false} wisorHeightFactor={2}>
            <div>
